...
export class MultiplayerHub {
  ...
  join(account,input={}){
    ...
    const mapProfile=text(input.mapProfile||'standard',40);if(!['standard','promods'].includes(mapProfile))throw new Error('Unbekanntes Kartenprofil');
    const room=[...this.sessions.values()].find(value=>value.game===game&&value.serverId===serverId);if(room&&room.mapProfile!==mapProfile)throw new Error(`Kartenprofil stimmt nicht überein: Server nutzt ${room.mapProfile}`);
    this.clean();const key=this.key(game,serverId,account.steamId),existing=this.sessions.get(key),session={
      ...
      clientVersion:text(input.clientVersion,30),pluginVersion:text(input.pluginVersion,30),mapProfile,sequence:0,joinedAt:existing?.joinedAt||new Date(this.now()).toISOString(),lastSeen:this.now(),state:null
