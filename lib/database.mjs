...
class MariaDatabase{
  ...
  async saveSessions(items){
    await this.pool.execute('DELETE FROM client_sessions');
    const [rows]=await this.pool.query('SELECT id FROM drivers');
    const driverIds=new Set(rows.map(row=>String(row.id)));
    for(const x of items){
      const driverId=String(x.vtcAccountId||'');
      const expires=new Date(x.expires);
      if(!x.hash||!driverIds.has(driverId)||!x.steamId||!x.displayName||Number.isNaN(expires.getTime()))continue;
      await this.pool.execute('INSERT INTO client_sessions(token_hash,driver_id,steam_id,display_name,role,expires_at) VALUES(?,?,?,?,?,?)',[x.hash,driverId,x.steamId,x.displayName,x.role||'driver',expires]);
    }
  }
