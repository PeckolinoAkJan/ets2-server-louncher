...
export async function createDatabase(env=process.env){
  ...
  if(!url)return new DisabledDatabase();
  const mysql=await import('mysql2/promise');
  const pool=mysql.createPool({uri:url,connectionLimit:Number(env.DB_POOL_SIZE||8),enableKeepAlive:true,timezone:'Z',charset:'utf8mb4'});
