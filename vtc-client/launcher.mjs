...
const RUNTIME_VERSION='0.9.10-dev';
...
function autoLoadDispatcherSlot(game,searchTerm=''){
  ...
  rmSync(resultFile,{force:true});const args=['-NoProfile','-ExecutionPolicy','Bypass','-File',script,'-Slot',String(slot),'-ResultFile',resultFile];if(searchTerm)args.push('-SearchTerm',searchTerm);const child=spawn('powershell.exe',args,{detached:true,stdio:'ignore',windowsHide:true});child.unref();return{slot,resultFile,searchTerm};
}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host}`);try{
  ...
  if(url.pathname==='/api/game/launch'&&req.method==='POST'){
    ...
    const launchState={game:input.game,server:live,startedAt:Date.now(),logOffset,searchId:live.searchId};launches.set(input.game,launchState);writeFileSync(activeLaunchFile,JSON.stringify(launchState,null,2));startTelemetry(input.game);const convoyJoin=prepareConvoyJoin(input.game,live),autoLoad=autoLoadDispatcherSlot(input.game,convoyJoin.searchTerm);
    return send(res,202,{ok:true,game:input.game,server:live,connection:'convoy-browser-pending',autoLoad,convoyJoin,searchId:live.searchId,message:autoLoad?`Spiel gestartet. VTC-Spielstand ${autoLoad.slot} wird automatisch geladen. Convoy-Suche ${convoyJoin.searchTerm} ist vorbereitet.`:`Spiel gestartet. Convoy-Suche ${convoyJoin.searchTerm} ist vorbereitet.`});
  }
